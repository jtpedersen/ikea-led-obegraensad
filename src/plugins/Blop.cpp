...
#include <chrono>
#include <thread>
void delay(int s) { std::this_thread::sleep_for(std::chrono::milliseconds(s)); }
#endif

#include "plugins/Blop.h"

BlobPlugin::BlobPlugin(PixelDisplay &display) : display(display) {
...
void BlobPlugin::loop() {
  ...
  for (uint8_t y = 0; y < ROWS; ++y) {
    for (uint8_t x = 0; x < COLS; ++x) {
      ...
      display.setPixel(x, y, 1, brightness);
    }
  }
...
const char *BlobPlugin::getName() const { return "Blobs"; }
